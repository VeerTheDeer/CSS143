...
public class ClassDesignIIDriver {
    ...
    /*
     * The primary difference is that when you pass a primitive to a method, you're passing the value, while with an object, you pass a reference to that object.
     *
     * No, the scope of a primitive doesn't change when passed to a method. It's passed by value, and changes are local to the method.
     *
     * The object's scope remains the same when passed to a method. The reference's scope changes within the method.
     *
     * The object's scope remains in the same location where it was created, but the reference's scope is local to the method it's passed to.
     */
}